package com.auak.agenty.agent

import android.content.Context
import android.content.Intent
import com.auak.agenty.App
import com.auak.agenty.controller.AppScanner
import com.auak.agenty.controller.DeviceController
import com.auak.agenty.data.ExecutionStep
import com.auak.agenty.skills.SkillManager
import com.auak.agenty.ui.OverlayService
import com.auak.agenty.vlm.VLMClient
...
class MobileAgent(
    ...
    private val skillManager: SkillManager? = try {
        SkillManager.getInstance().also {
            println("[AUAK智能体] SkillManager 已加载，共 ${it.getAllSkills().size} 个 Skills")
            // 设置 VLM 客户端用于意图匹配
            it.setVLMClient(vlmClient)
        }
    } catch (e: Exception) {
        println("[AUAK智能体] SkillManager 加载失败: ${e.message}")
        null
    }
    ...
    private suspend fun waitForUserTakeOver(message: String) = suspendCancellableCoroutine<Unit> { continuation ->
        com.auak.agenty.ui.OverlayService.showTakeOver(message) {
    ...
    private suspend fun waitForUserConfirm(message: String) = suspendCancellableCoroutine<Boolean> { continuation ->
        com.auak.agenty.ui.OverlayService.showConfirm(message) { confirmed ->
            if (continuation.isActive) {
